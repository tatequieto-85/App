...
// pasa a "terminado" solo, sin que haga falta guardarlo, en dos casos:
// cerrada a mano con "Terminar feria" (f.cerrada), o ya pasó su fecha de
// fin — esté en Participar o en Publicado, da igual (a pedido del
// usuario: "así esté publicado o participando, si ya pasó la fecha va en
// terminados"). Datos viejos sin f.estado (o con el "disponible" que
// traía la app vanilla, nunca usado acá) caen en "participar" por defecto.
export function estadoEfectivo(f) {
  if (f.cerrada || feriaHaTerminado(f)) return 'terminado';
  return f.estado === 'publicado' ? 'publicado' : 'participar';
}
